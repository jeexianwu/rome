 ...
 */
public final class DMDictionaryVectorizer extends AbstractJob implements Vectorizer {
	  private static final Logger log = LoggerFactory.getLogger(DMDictionaryVectorizer.class);
	  ...
	  //dictionary directory --- added by lucene ji
	  public static Path dictionarydir;
	  private static final String HDFSROOTRUL = "hdfs://192.168.127.130:8020/";
	  
	  /**
	   * Cannot be initialized. Use the static functions
	   */
	  private DMDictionaryVectorizer() {
	  }
	  ...
	  public static void createTermFrequencyVectors(Path input,
	    ...
	    //读dirctionary目录下的所有文件，并加入到dictionaryChunkszh
	    Configuration dicfilesConf = new Configuration();
	    FileSystem hdfs=FileSystem.get(dicfilesConf);
	    //Path path=new Path(HDFSROOTRUL+dictionaryDir);
	    FileStatus status[]=hdfs.listStatus(dictionarydir);
	    log.info("dictionary files count: "+status.length);
	    for(int i=0; i<status.length; i++){
	    	//System.out.println(status[i].getPath().toString());
	    	String dicPathStr = status[i].getPath().toString().split("8020")[1];
	    	log.info(dicPathStr);    	
	    	Path dicPath = new Path(dicPathStr);
	    	dictionaryChunks.add(dicPath);
	    	//get the number of keys in this dictionary
	    	SequenceFileIterator<?, ?> iterator = new SequenceFileIterator<Writable, Writable>(dicPath, true, dicfilesConf);
	    	int count = 0;
	    	while (iterator.hasNext()) {
	            count++;
	          }
	    	log.info("dict count: "+count);
	    	iterator.close();
	    	if (count > maxTermDimension[0]){
	    		maxTermDimension[0] = count;
	    	}
	    	
	    }
	    hdfs.close();
	    
	    //maxTermDimension[0] = 552945;
	  ...
	  private static void makePartialVectors(Path input,
	    ...
	    job.setJobName("DMDictionaryVectorizer::MakePartialVectors: input-folder: " + input
	                    + ", dictionary-file: " + dictionaryFilePath);
	    job.setJarByClass(DMDictionaryVectorizer.class);
	    ...
	    job.setReducerClass(DMTFPartialVectorReducer.class);
	  ...
	  public static void main(String[] args) throws Exception {
	    ToolRunner.run(new DMDictionaryVectorizer(), args);
	  }
	}