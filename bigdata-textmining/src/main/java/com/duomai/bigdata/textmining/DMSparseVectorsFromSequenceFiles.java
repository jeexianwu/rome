...
/**
 * Converts a given set of sequence files into SparseVectors using some given dictionary files
 */
public final class DMSparseVectorsFromSequenceFiles extends AbstractJob {

  private static final Logger log = LoggerFactory.getLogger(DMSparseVectorsFromSequenceFiles.class);

  public static void main(String[] args) throws Exception {
    ToolRunner.run(new DMSparseVectorsFromSequenceFiles(), args);
  }
  ...
  public int run(String[] args) throws Exception {
    ...
    Option dictionaryDirOpt = obuilder.withLongName("dictionaryDir").withArgument(
            abuilder.withName("dictionaryDir").withMinimum(1).withMaximum(1).create()).withDescription(
            "(Required) The gived dictionary files directory").withShortName("dd").create();
    ...
    Group group = gbuilder.withName("Options").withOption(minSupportOpt).withOption(analyzerNameOpt)
            ...
            .withOption(maxDFSigmaOpt).withOption(maxDFPercentOpt).withOption(weightOpt).withOption(powerOpt).withOption(dictionaryDirOpt)
    ...
    try {
      ...
      //TODO: get the directory, and to modify DMDictionaryVectorizer.dictionarydir
      Path dictionaryDir = new Path((String) cmdLine.getValue(dictionaryDirOpt));
      ...
      //TODO: move this into DMDictionaryVectorizer , and then fold SparseVectorsFrom with EncodedVectorsFrom
      ...
      String tfDirName = shouldPrune
              ? DMDictionaryVectorizer.DOCUMENT_VECTOR_OUTPUT_FOLDER + "-toprune"
              : DMDictionaryVectorizer.DOCUMENT_VECTOR_OUTPUT_FOLDER;
      log.info("Creating Term Frequency Vectors");
      if (processIdf) {
        DMDictionaryVectorizer.createTermFrequencyVectors(tokenizedPath,
                outputDir,
                tfDirName,
                conf,
                minSupport,
      ...
      if (shouldPrune) {
        ...
        Path prunedTFDir = new Path(outputDir, DMDictionaryVectorizer.DOCUMENT_VECTOR_OUTPUT_FOLDER);
        Path prunedPartialTFDir =
                new Path(outputDir, DMDictionaryVectorizer.DOCUMENT_VECTOR_OUTPUT_FOLDER + "-partial");
        log.info("Pruning");
        if (processIdf) {
      ...
      if (processIdf) {
        TFIDFConverter.processTfIdf(
                new Path(outputDir, DMDictionaryVectorizer.DOCUMENT_VECTOR_OUTPUT_FOLDER),
